...
export default function TicTacToe() {
    ...
    function getWinner(squares) {
        ...
        for (let i = 0; i < winningPatterns.length; i++) {
            ...
            if (squares[x] && squares[x] === squares[y] && squares[y] === squares[z] && squares[z]) {
                return squares[x];
            }
        }
        return null;
    ...
    return (
        <div className="tic-tac-toe-container">
            ...
            <h1>{status}</h1>
            <button type="button" class="btn btn-outline-dark" onClick={handleRestart}>Restart</button>
        </div>
    );
}